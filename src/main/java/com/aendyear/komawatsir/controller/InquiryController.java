...
@Controller
@RequestMapping("/api/inquiry/{userId}/{year}")
public class InquiryController {
    ...
    public ResponseEntity<List<InquiryItemDto>> getQuestionList(@PathVariable Integer userId, @PathVariable String year) {
        return ResponseEntity.ok(inquiryService.getQuestionList(userId, year));
    }

    @PostMapping
    @Operation(summary = "add inquiry question", description = "질문 추가하기")
    public ResponseEntity<InquiryItem> postInsertQuestion(@PathVariable Integer userId, @PathVariable String year, @RequestBody InquiryItemDto dto) {
        return ResponseEntity.ok(inquiryService.postInsertQuestion(userId, year, dto));
    }
